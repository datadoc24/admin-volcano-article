def train_func():
    ...
    # [3] Attach model to device and distributor.
    device = "cpu"
    model = Net().to(device)
...
for i in range(8):
  thread=threading.Thread(target=TrainingClient().create_job, kwargs={"name":"pytorch-t"+str(i),"train_func":train_func,"num_workers":4,"resources_per_worker":{"cpu":"1","memory":"500Mi"},})
